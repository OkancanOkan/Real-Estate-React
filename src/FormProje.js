...
import GetProjeDurumuCombo from "./GetProjeDurumuCombo";
import axios from "axios";
import React, { useState } from "react";

function FormProje() {
  const [txtProjectName, setTxtProjectName] = useState();
  const [cmbCity, setCmbCity] = useState();
  const [cmbProjectStatus, setCmbProjectStatus] = useState();

  const gonder = async () => {
    let requestBody = {
      txtName: txtProjectName,
      cmbCityName: cmbCity,
      cmbProjectStatusName: cmbProjectStatus,
    };
    const response = await axios.post(
      "https://private-3fc19-insaatapi1.apiary-mock.com/postProject",
      requestBody
    );
    if (response.data.Result === "İşlem başarıyla tamamlandı.") {
      ...
      // setResult('1');
      window.alert("Oke");
      // localStorage.setItem("ProjectName","Suryapı Antalya");
      // navigate("/");
                      ...
                      <div className="form-body">
                        <div className="form-group">
                          ...
                          <input
                            ...
                            id="txtName"
                            autoComplete="off"
                            onChange={(e) => setTxtProjectName(e.target.value)}
                            required
                          />
                        </div>
                        <div className="form-group">
                        ...
                        <div className="form-group">
                          ...
                          <GetProjeDurumuCombo
                            setProjectStatusID={setCmbProjectStatus}
                          />
                        </div>
                      </div>
